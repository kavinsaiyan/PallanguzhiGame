...
void DrawTamilText(const char *text, Vector2 position, Color color)
{
    ...
    for (unsigned int i = 0; i < glyph_count; i++)
    {
        ...
        // 🚨 CRITICAL Safety Check for fixed array 🚨
...
int main(void)
{
    InitWindow(800, 450, "Tamil Fixed");
    ...
    while (!WindowShouldClose())
    {
		...
		if(!drawOnce)
		{
			...
			DrawTamilText("வணக்கம் உலகம்", (Vector2){100, 100}, BLACK);
            DrawText("Works!", 100, 200, 20, DARKGRAY);
		}
        EndDrawing();
    }
    UnloadComplexText();
